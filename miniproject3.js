...
// === Updated Usage - Both students work on both assignments ===
...
// Both students work on both A1 and A2
classList.releaseAssignmentsParallel(["A1", "A2"]).then(() => {
  // Alice works on both assignments
  s1.startWorking("A1");
  s1.startWorking("A2");
  
  // Bob works on both assignments  
  s2.startWorking("A1");
  s2.startWorking("A2");

  // Send reminders for A1 after 200ms
...
setTimeout(() => {
    console.log('\n--- Final Results ---');
    console.log(`Alice's overall grade: ${s1.getGrade().toFixed(2)}`);
    console.log(`Bob's overall grade: ${s2.getGrade().toFixed(2)}`);
    console.log(`Alice's A1 status: ${s1.getAssignmentStatus("A1")}`);
    console.log(`Alice's A2 status: ${s1.getAssignmentStatus("A2")}`);
    console.log(`Bob's A1 status: ${s2.getAssignmentStatus("A1")}`);
